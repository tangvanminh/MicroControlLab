...
void getKeyInput(){
	...
	if(debounceButtonBuffer1[2]==debounceButtonBuffer2[2]
	   && debounceButtonBuffer2[2] == debounceButtonBuffer3[2]){
		if(debounceButtonBuffer3[2] == BUTTON_PRESSED){
			button2_flag = 1;
			button2_press_count--;
			if(button2_press_count == 0){
				button2_press_count = 25;
				debounceButtonBuffer3[2] = BUTTON_RELEASED;
			}
		}
	}else{
		if(button2_press_count != 25 && button2_press_count != 100){
